        ...
        if (show && config->misc.jumpStats.enabled)
        {
            ...
            if (jumps > 2)
                memory->clientMode->getHudChat()->printf(0,
                    " \x0C\u2022ketamine\u2022\x01 %c%s: %.2f units \x01[\x05%d\x01 Strafes | \x05%.0f\x01 Pre | \x05%.0f\x01 Max | \x05%.1f\x01 Height | \x05%d\x01 Bhops | \x05%.0f\x01 Sync]",
                    color, jump.c_str(),
                ...
                memory->clientMode->getHudChat()->printf(0,
                    " \x0C\u2022ketamine\u2022\x01 %c%s: %.2f units \x01[\x05%d\x01 Strafes | \x05%.0f\x01 Pre | \x05%.0f\x01 Max | \x05%.1f\x01 Height | \x05%.0f\x01 Sync]",
                    color, jump.c_str(),
...
void Misc::watermark() noexcept
{
    ...
    ImGui::Text("ketamine | %d fps | %d ms", frameRate != 0.0f ? static_cast<int>(1 / frameRate) : 0, GameData::getNetOutgoingLatency());
    ImGui::End();
}
...
void Misc::voteRevealer(GameEvent& event) noexcept
{
    ...
    memory->clientMode->getHudChat()->printf(0, " \x0C\u2022ketamine\u2022 %c%s\x01 voted %c%s\x01", isLocal ? '\x01' : color, isLocal ? "You" : entity->getPlayerName().c_str(), color, votedYes ? "Yes" : "No");
}
